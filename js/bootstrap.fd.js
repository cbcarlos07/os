...
$.FileDialog = function FileDialog(userOptions) {
    ...
    var loadFile = function(f) {
        ...
        var progress = $([
            ...
            "    <span class='glyphicon glyphicon-file'></span>&nbsp;<span class='filename'>" + f.name+"</span>",
            "</div>",
    ...
    $(".bfd-ok", modal).on("click.bfd", function() {
        ...
        for ( i = 0; i < event.files.length; i++){
            //anexo.push( event.files[i].name );
            anexo.name = event.files[i].name;
            vetor.push( {
...
$.FileDialog.defaults = {
    ...
    "drag_message": "Arraste arquivos aqui",
    "dropheight": 400,
    "error_message": "Um erro ocorreu enquanto carregando o arquivo",
    "multiple": true,
    "ok_button": "OK",
